(function ($) {
  ...
  function reloadAjax($off) {
    console.log("reloadAjax Loading sequence started...");
  ...
  function effctStickyNavBar() {
    console.log("effctStickyNavBar Loading sequence started...");
   ...
   */
  function preloader() {
    console.log("0 Custom.js running.");
    ...
    console.log("Wind: ", wind, "preloader: ", preloader);
    wind.on("load", function () {
      try {
        clearInterval(timer);
        console.log("1 Custom.js running.");
        ...
        TweenMax.fromTo(
          ...
          { width: "95%" },
          {
            ...
            onComplete: function () {
              console.log("TweenMax completed");
              ...
              TweenMax.to(preloader_after, 1, {
                ...
                delay: 0.7,
                onComplete: function () {
                  console.log("2 Custom.js running.");
  ...
  function effectScroller() {
    ...
    return {
      ...
      getScrollbar: function ($id) {
        if ($id === undefined) {
          console.log("myScroll elem: ", myScrollbar)
          console.log("default scrollbar: ", Scrollbar.get(myScrollbar))
          return Scrollbar.get(myScrollbar);
        }
        const element = document.querySelector($id)
        return Scrollbar.get(element);
      },
  ...
  function navBar() {
    ...
    wind.on("load", function () {
      ...
      menu.on("click", function () {
        console.log("inside func", menu, body);
        body.toggleClass($nav_active);
        console.log("body has class nav-active: ", body.hasClass("nav-active"));
      });
      console.log("outside func", menu);
  ...
  function LoadingPage() {
    const filter = $("#filteringP");
    const filter1 = $("#filtering1");
    const filter2 = $("#filtering2");
    const filter3 = $("#filtering3");
    const gallery = $("#galleryP");
    const gallery1 = $("#gallery1");
    const gallery2 = $("#gallery2");
    const gallery3 = $("#gallery3");
    /* isotope
                  -------------------------------------------------------*/
    ...
    var $gallery1 = gallery1.isotope({
      // options
      itemSelector: ".item",
      transitionDuration: "0.5s",
    });

    var $gallery2 = gallery2.isotope({
      // options
      itemSelector: ".item",
      transitionDuration: "0.5s",
    });

    var $gallery3 = gallery3.isotope({
      // options
      itemSelector: ".item",
      transitionDuration: "0.5s",
    });
    ...
    filter2.on("click", "button", function () {
      ...
      // setTimeout(function () {
      //   TweenLite.to(sc, 1.5, {
      //     scrollTo: $gallery2.offset().top - 150,
      //     ease: Expo.easeInOut,
      //   });
      // }, 500);
    });
    ...
    /* Apply filtering on document load */
    $(document).ready(function () {
      const defaultFilter1 =
        filter1.find("button.active").attr("data-filter") || "*";

      $gallery1.isotope({
        filter: defaultFilter1,
      });

      const defaultFilter2 =
        filter2.find("button.active").attr("data-filter") || "*";

      $gallery2.isotope({
        filter: defaultFilter2,
      });

      const defaultFilter3 =
        filter3.find("button.active").attr("data-filter") || "*";

      $gallery3.isotope({
        filter: defaultFilter3,
      });

      // Scroll to gallery on load if required
      // let sc = wind;
      // if (effectScroll.isScroller(true)) sc = effectScroll.getScrollbar();
      ...
      //   setTimeout(function () {
      //     TweenLite.to(sc, 1.5, {
      //       scrollTo: $gallery1.offset().top - 150,
      //       ease: Expo.easeInOut,
      //     });
      //   }, 500);
    });
